...
Script to create h5 material files for SEM3D
    ...
    with step [20x20x20]
        ...
        python3 generate_h5_materials.py @@tag linear_gradient @@pfx example @@dir z @@xlim -100.0 100.0 @@ylim -100.0 100pro.0 @zlim -100.0 100.0
            @@step 20 20 20
"""
# Required modules
import argparse
...
trnsp = (2,1,0)
dirs_prop = {"la":"Lambda","mu":"Mu","ds":"Rho","vp":"Vp","vs":"Vs"}
...
def write_h5(pfx,prop,mat,lims,xdmf=True):
    for v in prop:
        with h5py.File("{}_{}.h5".format(pfx,v),"w") as fid:
            #samples = fid.create_dataset(dict_propo[prop],mat[v].shape,data=mat[v])
            samples = fid.create_dataset('samples',mat[v].shape,data=mat[v]) # chunks=tuple([l//10 for l in mat[v].shape]))
...
if __name__=='__main__':
    ...
    parser.add_argument('@@step',type=float,nargs='*',default=[3,3,101],help="Numbers of points per direction [nx ny nz]")
