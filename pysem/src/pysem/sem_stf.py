...
taglabel={'spice_bench':r'$1+(1-f_C(t-t_0))e^{-f_C(t-t_0)}$'}
def nextpow2(i):
    """
...
def plot_write(stf,sff,tag):
    np.savetxt('{}_stf.txt'.format(tag),stf,fmt='%.15f', delimiter=' ')
    np.savetxt('{}_sff.txt'.format(tag),sff.view(float)[:,[0]+list(range(2,4))],fmt='%.15f', delimiter=' ')
    plt.figure(figsize=(12,5))
    plt.plot(stf[:,0],stf[:,1],linewidth=3,label=tag)
    plt.xlabel(r'$\mathbf{t [s]}$',fontsize=14)
    plt.ylabel(r'$\mathbf{STF [m]}$',fontsize=14)
    plt.legend()
    plt.savefig('{}_stf.png'.format(tag),bbox_inches='tight')
    plt.close()
...
if __name__== '__main__':
    ...
    for i in range(len(opt['model'])):
        ...
        try:
            ...
            pass
        tag = "md{}-ts{:>.2f}s-fc{:>.2f}Hz".format(md,round(ts,2),round(fc,2))
        if 'gaussian' in md:
            wd = 1./fc
